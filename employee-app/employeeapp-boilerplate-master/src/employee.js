...
const getEmployeeOverFifty = (employeeData) => new Promise((resolve, reject) => {
  ...
  resolve(employeeData.filter(employee=>employee.employeeAge>50).length);
})
...
const getTotalNoOfDaysSinceJoining = (employeeData) => new Promise((resolve, reject) => {
  ...
  resolve(employeeData.map(employee => Math.floor((new Date().getTime() - new Date(employee.dateOfJoining).getTime()) / (1000 * 3600 * 24))));
}
